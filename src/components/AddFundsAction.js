...
class AddFundsAction extends Component {
  constructor(props) {
    super(props);
  ...
  async handleSubmit() {
    ...
    try {
      await BalanceService.add(Number(this.state.value));
    } catch (error) {
      this.setState({ error });
      return;
    }
    ...
    return (
      <div className="AddFundsAction">
        ...
        <Dialog open={this.state.open} onClose={this.handleClose} aria-labelledby="form-dialog-title">
          <DialogTitle id="form-dialog-title">Add funds to wallet</DialogTitle>
          <DialogContent>
            {error}

            <DialogContentText>
              Enter the amount you wish to transfer from your linked bank account to your wallet's balance.
            </DialogContentText>
